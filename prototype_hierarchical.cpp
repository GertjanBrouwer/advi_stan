...
namespace eight = _8schools_model_namespace; 
...
int main() {
    ...
    eight::_8schools_model* model;
    std::ifstream in("data/8schools_hierarchical.json");
    stan::json::json_data data_context(in);
    unsigned int seed = 1234;
    model = new eight::_8schools_model(data_context, seed, &std::cout);
    ...
    std::vector<double> m_mu(10, 0.0);
    std::vector<double> v_mu(10, 0.0);
    std::vector<double> m_hat_mu(10, 0.0);
    std::vector<double> v_hat_mu(10, 0.0);
    std::vector<double> m_omega(10, 0.0);
    std::vector<double> v_omega(10, 0.0);
    std::vector<double> m_hat_omega(10, 0.0);
    std::vector<double> v_hat_omega(10, 0.0);
    ...
    Eigen::VectorXd mu = Eigen::VectorXd::Zero(10);
    Eigen::VectorXd omega = Eigen::VectorXd::Ones(10);
    Eigen::VectorXd mu_grad = Eigen::VectorXd::Zero(10);
    Eigen::VectorXd omega_grad = Eigen::VectorXd::Zero(10);
    Eigen::VectorXd gradient = Eigen::VectorXd::Zero(10);

    double temp_lp = 0.0;
    Eigen::VectorXd eta = Eigen::VectorXd::Zero(10);
    Eigen::VectorXd zeta = Eigen::VectorXd::Zero(10);
    Eigen::Matrix<stan::math::var, -1, 1> eigen_x(10);
    // eigen_x[0] = 0; // TODO: Stan inits uniformly [-2,2]
    // This is not used bit still required as an input.
    Eigen::Matrix<int, -1, 1> eigen_int(10);
    // eigen_int[0] = 1;
    ...
    for (int t = 0; t < 100; t++)
    {
        ...
        for (int i = 0; i < m; i++) {
            ...
            {
                eigen_x[i] = zeta[i];
                // std::cout << eigen_x[i] << " eigen_x" << std::endl;
                // std::cout << zeta[i] << " zeta" << std::endl;
            }
            ...
            //std::cout << eigen_x[0].adj() << " " << eigen_x[1].adj() << std::endl;
            //working_gradient = eigen_x.adj();
            //mu_grad += working_gradient;
            for (int i = 0; i < 10; i++)
            {
                mu_grad[i] += eigen_x[i].adj();
                omega_grad[i] += eigen_x[i].adj() * eta[i];
                /* code */
            }
            
            // mu_grad += eigen_x.adj();
            // //std::cout << mu_grad[0] << " " << mu_grad[1] << std::endl;
            // omega_grad.array() += eigen_x.adj().array().cwiseProduct(eta.array());
        }
        ...
        for (int i = 0; i < 10; i++)
        {
            m_mu[i] = b1 * m_mu[i] + (1 - b1) * mu_grad[i];
            v_mu[i] = b2 * v_mu[i] + (1 - b2) * pow(mu_grad[i], 2);
            m_hat_mu[i] = m_mu[i] / (1 - pow(b1, t + 1));
            v_hat_mu[i] = v_mu[i] / (1 - pow(b2, t + 1));
            //mu_step = step_size * m_hat_mu / (sqrt(v_hat_mu) + eps);
            mu[i] += step_size * m_hat_mu[i] / (sqrt(v_hat_mu[i]) + eps);

            m_omega[i] = b1 * m_omega[i] + (1 - b1) * omega_grad[i];
            v_omega[i] = b2 * v_omega[i] + (1 - b2) * pow(omega_grad[i], 2);
            m_hat_omega[i] = m_omega[i] / (1 - pow(b1, t + 1));
            v_hat_omega[i] = v_omega[i] / (1 - pow(b2, t + 1));
            //omega_step = step_size * m_hat_omega / (sqrt(v_hat_omega) + eps);
            omega[i] += step_size * m_hat_omega[i] / (sqrt(v_hat_omega[i]) + eps);
        }
    ...
    for (int i = 0; i < 10; i++)
    {
        std::cout << "mu" << i << ": " << mu[i] << " omega" << i << ":" << omega[i] << std::endl;
    }
